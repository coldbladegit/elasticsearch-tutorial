...
public class IndexService {
    ...
    public IndexResponse createIndex(String index, IndexType indexType) {
        return createIndex(index, indexType, null);
    }

    public IndexResponse createIndex(String index, IndexType indexType, @Nullable String id) {
        try (TransportClient client = new PreBuiltTransportClient(setting.getSettings())
            .addTransportAddress(setting.getAddress())) {
            return client.prepareIndex(index, indexType.name(), id).get();
        }
    }
}
